...
int CameraManager::searchCameras()
{
    ...
    try {
        ...
        for (size_t i = 0; i < devices.size(); i++) {
            ...
            const char *sn = String_t(devices[i].GetFriendlyName().c_str());
            mCameraList << QString::fromStdString(sn);
            qDebug() << "Using camera " << i << ": " << devices[i].GetModelName() << " ("
                     << devices[i].GetIpAddress() << ")" << endl;
        }
...
bool CameraManager::registerHandler(CInstantCamera *camera)
{
    try {
        ...
        camera->RegisterImageEventHandler(this,
        ...
        camera->RegisterConfiguration(this, Pylon::RegistrationMode_ReplaceAll, Pylon::Cleanup_None);
    } catch (const Pylon::GenericException& e) {
